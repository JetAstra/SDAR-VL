...
class FinetuningArguments(
    ...
    noise_scheduler_type: str = field(
        default="uniform",
        metadata={"help": "噪声调度器的类型。可选值为 'uniform', 'beta', 'clamp'"},
    )
    noise_min: float = field(
        default=0.0,
        metadata={"help": "clamp scheduler 的 min value"},
    )
    noise_max: float = field(
        default=1.0,
        metadata={"help": "clamp scheduler 的 max value"},
    )
    scheduler_warmup_steps: int = field(
        default=1000,
        metadata={"help": "在调度器中用于线性预热(warmup)的总训练步数的比例。"},
    )
    scheduler_target_mean: float = field(
        default=0.8,
        metadata={"help": "Beta scheduler 的目标均值 (Target mean for the beta noise scheduler.)"},
    )
    scheduler_start_C: float = field(
        default=2.0,
        metadata={"help": "Beta scheduler 参数 C 的起始值 (The starting value for the C parameter.)"},
    )
    scheduler_end_C: float = field(
        default=50.0,
        metadata={"help": "Beta scheduler 参数 C 的结束值 (The ending value for the C parameter.)"},
    )
