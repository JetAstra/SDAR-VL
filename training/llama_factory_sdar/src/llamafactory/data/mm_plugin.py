...
class MMPluginMixin:
    ...
    def _regularize_videos(self, videos: list["VideoInput"], **kwargs) -> dict[str, list[list["ImageObject"]]]:
        ...
        for video in videos:
            ...
            if _check_video_is_nested_images(video):
                ...
                frames = [os.path.join(video, frame) for frame in frames]
                sample_frames = min(len(frames), kwargs['video_maxlen'])
                sample_indices = np.linspace(0, len(frames) - 1, sample_frames).astype(np.int32)
    ...
    def _get_mm_inputs(
        ...
        if len(videos) != 0:
            ...
            params = inspect.signature(video_processor.preprocess).parameters
            if "videos" in params and "images" in params:  # e.g., for Qwen2-VL, Video-LLaVA
                mm_inputs.update(video_processor(images=None, videos=videos, return_tensors="pt"))
            elif isinstance(video_processor, LlavaOnevisionVideoProcessor):  # e.g., for LlavaOneVision
                mm_inputs.update(video_processor(videos=videos))
            elif "videos" in params:  # e.g., for Llava-Next-Video
                mm_inputs.update(video_processor(videos=videos, return_tensors="pt"))
            elif "images" in params: # Fallback for image processors handling video frames
                print("Warning: Using an image_processor to process video frames.")
                mm_inputs.update(video_processor(images=videos, return_tensors="pt"))
            else:
                raise TypeError(
                    f"The processor '{type(video_processor).__name__}' has a `preprocess` method "
                    "that does not accept 'videos' or 'images' as a parameter, "
                    "and cannot be used for video processing."
                )
...
class SDARVPlugin(BasePlugin):
    ...
    def process_messages(
        ...
        # --- 视频处理部分的修正 ---
        if self.expand_mm_tokens and "pixel_values_videos" in mm_inputs:
            video_features_iter = iter(mm_inputs.get("pixel_values_videos"))
        ...
        for message in messages:
            ...
            while VIDEO_PLACEHOLDER in content:
                if self.expand_mm_tokens and "pixel_values_videos" in mm_inputs:
                    try:
                        next_video_features = next(video_features_iter)
                        height, width = get_image_size(next_video_features[0])
                        num_frames = next_video_features.shape[0]
                        patches_height_width = int(math.sqrt(processor.num_image_tokens))
                        pooled_height_width = math.ceil(patches_height_width / 2)
                        video_seqlen = (num_frames * pooled_height_width * pooled_height_width) + 1  # +1 for newline token
                    except StopIteration:
                        # 如果文本中的 <video> 比实际视频文件多，抛出错误
                        raise ValueError("Mismatch: More VIDEO_PLACEHOLDERs in text than available video inputs.")
                else:
                    video_seqlen = 1 # 如果不扩展token，则长度为1
                content = content.replace(VIDEO_PLACEHOLDER, "{{video}}" * video_seqlen, 1)
